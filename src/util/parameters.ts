...
class Parameters<T> {
  ...
  static _isPrimitive(value: any) {
    return Parameters.PRIMITIVE_TYPES.some((Primitive) => {
  ...
  static _cloneObject(params: any) {
    return _.transform(params, function (result: { [key: string]: any }, value: any, key: string) {
    ...
    for (const key in filters) {
      ...
      if (_.isArray(filtersArray) && (param = this._fetch(key))) {
        ...
        if (_.isArray(param._params) || (isArrObj = Object.keys(param._params).every(function (i) { return !_.isNaN(Number(i)); }))) {
          if (isArrObj) {
            params[key] = _.transform(param._params, function (result: { [key: string]: any }, value, key: string) {
